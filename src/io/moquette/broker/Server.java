...
public class Server {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String config_file=flags.getString("-f","config-file",null,"configuration file");
        int port=flags.getInteger("-p","tcp-port",-1,"server TCP port");
        Bridge.DEFAULT_QOS=flags.getInteger("-q","qos",Bridge.DEFAULT_QOS,"internal QoS value");
        boolean exit=flags.getBoolean("-x","prompt to exit");
        final Server server = new Server(port);
        if (config_file!=null) server.startServer(new File(config_file));
        else server.startServer();
        //System.out.println("Server started, version 0.13-SNAPSHOT");
        System.out.println("Modified MQTT broker version 0.13-SNAPSHOT started on port "+port);
   ...
   /** Creates a new server. 
    ...
    public void startServer(IConfig config, List<? extends InterceptHandler> handlers, ISslContextCreator sslCtxCreator,
        ...
        if (port>0) config.setProperty(BrokerConstants.PORT_PROPERTY_NAME,String.valueOf(port));
        else port=Integer.parseInt(config.getProperty(BrokerConstants.PORT_PROPERTY_NAME));
        ...
        dispatcher = new PostOffice(port, subscriptions, retainedRepository, sessions, interceptor, authorizator);
