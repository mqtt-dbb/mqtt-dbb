...
public class ConnectProcessor implements RequestProcessor {
    ...
    private boolean authentication(String clientId,String username,byte[] password){
        if (username!=null && AccessToken.containsToken(username)) {
        	SysUtils.log(this,"processConnect(): access-token: "+username);
        	boolean is_auth=false;
			try {
				is_auth=AuthServer.verifyAccessToken(username);
			}
			catch (SignatureException e) {
				e.printStackTrace();
			}
        	SysUtils.log(this,"authentication(): verify: "+is_auth);
        	return is_auth;
        }
        else return this.connectPermission.authentication(clientId,username,password);
    }
